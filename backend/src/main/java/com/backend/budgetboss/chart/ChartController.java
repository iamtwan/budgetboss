...
public class ChartController {
	...
	public ResponseEntity<List<ChartResponse>> getChartData(@AuthenticationPrincipal UserPrinciple principle) {
	...
	public ResponseEntity<ChartTransactionsResponse> getTransactionsForMonth(@AuthenticationPrincipal UserPrinciple principle, @PathVariable Month month) {
		logger.info("/api/charts/{month} GET request received");
		ChartTransactionsResponse response = chartService.getTransactionsForMonth(principle.getUser(), month);
		logger.info("/api/charts/{month} got all data: {}", response.getTransactions().size());
